...
class SharedDirectory():
    def __init__(self, share_key):
        ...
        self.tree.create_node(tag='/', identifier='/', parent=None, data={'is_dir':True, 'size':0})
    ...
    def get_dir(self, path='/'):
        ...
        r = requests.get('https://cloud.tsinghua.edu.cn/api/v2.1/share-links/{}/dirents/?path={}'.format(self.share_key, urllib.parse.quote(path)))
    ...
    def download(self, save_dir):
        ...
        for node_id in self.tree.expand_tree():
            ...
            # 如果是文件，就下载到对应的位置
                ...
                file_url = download_url.format(self.share_key, urllib.parse.quote(self.tree[node_id].identifier))

                try:
                    r = requests.get(file_url, headers='', stream=True)
